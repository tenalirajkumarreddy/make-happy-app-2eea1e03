-- Migration: Fix edit_sale Stock Reversal Logic
-- Date: 2026-06-14
-- Priority: P0 - CRITICAL (Data Integrity / Stock Leak)
--
-- Issue Fixed:
-- When an admin edits an agent's sale, stock was reversed to the original agent
-- but new deduction was from the admin (who likely has no staff_stock).
-- This created a stock leak where the agent kept inflated stock.
--
-- Fix:
-- - If editor is same as original recorder: reverse to original, deduct from editor
-- - If editor is different: reverse to warehouse, deduct from editor's staff_stock (or warehouse)
...
BEGIN
    ...
    -- Check if editor has staff stock
    SELECT EXISTS (SELECT 1 FROM public.staff_stock WHERE user_id = p_recorded_by) INTO v_has_staff_stock_editor;
    
    -- Check if original recorder has staff stock
    SELECT EXISTS (SELECT 1 FROM public.staff_stock WHERE user_id = v_orig.recorded_by) INTO v_has_staff_stock_original;

    -- ── REVERSE ORIGINAL SALE ──────────────────────────────────────────────
    -- FIXED: Only reverse to original recorder if editor is the same user
    -- If editor is different (e.g., admin editing agent's sale), reverse to warehouse
    FOR v_orig_item IN SELECT * FROM public.sale_items WHERE sale_id = p_original_sale_id
    LOOP
        -- ✅ FIXED LOGIC:
        -- If same user: reverse to their staff_stock
        -- If different user: reverse to warehouse (product_stock)
        IF p_recorded_by = v_orig.recorded_by AND v_has_staff_stock_original THEN
            -- Same user editing their own sale - reverse to their staff_stock
            ...
            END IF;
        ELSE
            -- Different user editing - reverse to warehouse stock to avoid stock leak
    ...
    LOOP
        ...
        IF v_caller_role = 'agent' THEN
            ...
            END IF;
        ELSE
            -- Other roles: Try staff stock first, fallback to warehouse
            IF v_has_staff_stock_editor THEN
                SELECT ss.quantity INTO v_staff_available_stock
                FROM public.staff_stock ss
                WHERE ss.user_id = p_recorded_by
                  AND ss.product_id = v_product_id
            ...
            IF NOT v_has_staff_stock_editor OR v_staff_available_stock < v_quantity THEN
