-- Fix record_transaction RPC to resolve and set warehouse_id,
-- backfill existing records, and fix RLS policies.

-- 1. Fix record_transaction RPC to resolve and set warehouse_id
-- Matches the pattern from record_sale RPC
...
-- 2. Backfill warehouse_id on existing transactions where it's NULL
UPDATE public.transactions t
SET warehouse_id = COALESCE(
  (SELECT warehouse_id FROM public.user_roles WHERE user_id = t.recorded_by AND warehouse_id IS NOT NULL LIMIT 1),
  (SELECT id FROM public.warehouses LIMIT 1)
)
WHERE t.warehouse_id IS NULL;

-- 3. Fix RLS policy for transactions SELECT to include marketer and customer
  ...
  USING (
    EXISTS (
      ...
      WHERE user_id = auth.uid()
        AND role IN ('super_admin', 'manager', 'agent', 'operator', 'marketer')
    )
    OR
    EXISTS (
      SELECT 1 FROM public.customers c
      WHERE c.user_id = auth.uid() AND c.id = customer_id
    )
  );
