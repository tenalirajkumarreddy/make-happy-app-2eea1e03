-- Migration: Harden Sales, Returns, and Order Fulfillment RPCs
-- Date: 2026-05-30

-- 1. Upgrade check_stock_availability to disable warehouse fallback for agents
...
-- 2. Drop legacy record_sale functions
...
-- Recreate record_sale with optional p_fulfilled_order_id and hardened role-based checks
...
BEGIN
    ...
    LOOP
        ...
        IF v_caller_role = 'agent' THEN
            ...
            END IF;
        ELSE
            -- Other roles: Try staff stock first, fallback to warehouse stock
    ...
    -- Insert sale (storing fulfilled_order_id)
...
-- 3. Upgrade edit_sale function: add same-day locks and agent stock check
...
BEGIN
    ...
    -- (Check original recorder's staff stock, not editor's)
...
-- 4. Upgrade record_sale_return to enforce same-day lockout, full-only returns, and pure wastage routing
