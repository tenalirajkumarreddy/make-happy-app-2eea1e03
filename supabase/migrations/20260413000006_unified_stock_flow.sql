-- ============================================================================
-- MIGRATION: Unified Stock Flow with Return Workflow
-- ============================================================================
-- Adds status column to stock_transfers and creates unified transfer/return functions
...
BEGIN
  ...
  IF p_approved THEN
    ...
    IF p_difference > 0 THEN
      ...
      ELSE
        -- Keep with user - staff keeps the remaining stock
        -- (No action needed, transfer already deducted what was returned)
        INSERT INTO staff_performance_logs (
          user_id,
          log_type,
          reference_id,
          ...
          v_staff_user_id,
          'stock_return_variance',
          p_transfer_id,
          v_transfer.product_id,
          v_transfer.quantity,
          p_actual_quantity,
          p_difference,
          'Variance kept with staff: ' || p_notes,
          p_reviewed_by
        );
      END IF;
    END IF;
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION public.record_stock_transfer TO authenticated;
GRANT EXECUTE ON FUNCTION public.process_stock_return TO authenticated;
GRANT EXECUTE ON FUNCTION public.execute_stock_transfer TO authenticated;
