-- Migration: Fix update_transaction updated_by to use current user
-- Date: 2026-06-15
-- Priority: P0 - CRITICAL (Audit Trail Integrity)
--
-- Issue: update_transaction sets updated_by = v_txn.recorded_by (original recorder)
-- instead of the current user (v_user_id) who is actually performing the edit.
-- Fix: Use v_user_id (auth.uid()) for updated_by.

DROP FUNCTION IF EXISTS public.update_transaction(UUID, NUMERIC, NUMERIC, TEXT);
...
BEGIN
    ...
    SET cash_amount  = COALESCE(p_cash_amount, 0),
        ...
        updated_at   = NOW(),
        updated_by   = v_user_id  -- ✅ FIXED: was v_txn.recorded_by
...
GRANT EXECUTE ON FUNCTION public.update_transaction(UUID, NUMERIC, NUMERIC, TEXT) TO authenticated;