...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!session_token || !otp_code) {
      console.log('DEBUG: missing session_token or otp_code', { session_token: !!session_token, otp_code: !!otp_code })
      return new Response(
    ...
    console.log('DEBUG: fetched otpSession', { session_token, otpSession: !!otpSession, fetchError: fetchError?.message })
    ...
    if (!otpSession) {
      console.log('DEBUG: no otpSession found for', session_token)
      return new Response(
    ...
    if (session.otp_code !== otp_code.trim()) {
      console.log('DEBUG: OTP mismatch', { expected: session.otp_code, got: otp_code.trim() })
