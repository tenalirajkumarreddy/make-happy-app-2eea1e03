...
export function AgentHome({
  ...
  useEffect(() => {
    const initLocation = async () => {
      ...
      if (isNativeApp()) {
        ...
        if (!hasPermission) {
          toast.error(
            "Location permission required for nearby stores. Please enable in settings.",
          );
          return;
        }
      }
  ...
  const { data: activeSession } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("route_sessions")
        .select(
          "*, routes(name, stores(id, name, display_id, photo_url, address, lat, lng, store_order, phone, outstanding, route_id, store_type_id, customer_id, customers(name), store_types(name), routes(name)))",
        )
        .eq("user_id", user!.id)
        .eq("status", "active")
        .maybeSingle();
  ...
  const totalSales =
    salesData?.reduce((sum, row) => sum + (row.total_amount ?? 0), 0) ?? 0;
  const cashSales =
    salesData?.reduce((sum, row) => sum + (row.cash_amount ?? 0), 0) ?? 0;
  const upiSales =
    salesData?.reduce((sum, row) => sum + (row.upi_amount ?? 0), 0) ?? 0;
  const cashCollected =
    txData?.reduce((sum, row) => sum + (row.cash_amount ?? 0), 0) ?? 0;
  const upiCollected =
    txData?.reduce((sum, row) => sum + (row.upi_amount ?? 0), 0) ?? 0;

  const routeStores: RouteStoreLite[] = activeSession?.routes?.stores ?? [];
  const sortedStores = [...routeStores].sort(
    (left, right) => (left.store_order ?? 0) - (right.store_order ?? 0),
  );
  const unvisitedStores = sortedStores.filter(
    (store) => !visits?.has(store.id),
  );
  ...
  const visitedCount = sortedStores.filter((store) =>
    visits?.has(store.id),
  ).length;
  const progressPct = routeStores.length
    ...
    <div className="pb-6">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-muted-foreground">
                ...
                <strong className="text-foreground">
                  ₹{(cashSales + cashCollected).toLocaleString("en-IN")}
                </strong>
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-muted-foreground">
                UPI{" "}
                <strong className="text-foreground">
                  ₹{(upiSales + upiCollected).toLocaleString("en-IN")}
                </strong>
              </span>
            </div>
          </div>
      ...
      {activeSession ? (
        <div className="px-4 mt-5">
          ...
          <div className="rounded-2xl bg-card text-card-foreground border border-border shadow-sm overflow-hidden">
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/30 dark:to-indigo-900/30 px-4 py-3 flex items-center justify-between">
              <div>
                <p className="font-semibold text-foreground text-base">
                  {activeSession?.routes?.name ?? "Route"}
                </p>
            ...
            <div className="px-4 py-3">
              <div className="h-2.5 bg-muted hover:bg-accent rounded-full overflow-hidden">
                <div
              ...
              <div className="flex justify-between mt-1.5">
                ...
                <span className="text-[11px] text-muted-foreground">
                  {routeStores.length - visitedCount} remaining
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="px-4 mt-5">
          <div className="rounded-2xl border-2 border-dashed border-border p-5 text-center bg-muted/30">
          ...
          <div className="rounded-2xl bg-card text-card-foreground border border-border shadow-sm p-4">
            <div className="flex items-start gap-3">
              <button
                ...
                onClick={() => onOpenStore(nextStore)}
              >
                {nextStore.photo_url ? (
                  <img
                    src={nextStore.photo_url}
                    alt={nextStore.name}
                    loading="lazy"
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <Badge
                    variant="outline"
                    className="text-[10px] shrink-0 border-orange-200 text-orange-600 dark:border-orange-700 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/20"
                  >
                    Up Next
                  </Badge>
                </div>
            ...
            <div className="grid grid-cols-3 gap-2 mt-3">
              <Button
                size="sm"
                className="h-9 rounded-xl gap-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold"
                onClick={() => openDirections(nextStore)}
              >
              ...
              <Button
                size="sm"
                variant="outline"
                className="h-9 rounded-xl gap-1.5 text-xs font-semibold border-border"
                onClick={() => window.open(`tel:${nextStore.phone}`, "_self")}
                disabled={!nextStore.phone}
              >
            ...
            <div className="grid grid-cols-3 gap-2 mt-2">
              <Button
                ...
                variant="outline"
                className="h-9 rounded-xl gap-1.5 text-xs font-semibold"
                onClick={() => onGoRecord(nextStore, "sale")}
              >
              ...
              <Button
                ...
                variant="outline"
                className="h-9 rounded-xl gap-1.5 text-xs font-semibold"
                onClick={() => onGoRecord(nextStore, "payment")}
              >
      ...
      <div className="px-4 grid grid-cols-2 gap-3 mb-6 mt-4">
        <Button
          variant="outline"
          className="h-20 flex flex-col items-center justify-center gap-2 border-border shadow-sm hover:bg-muted dark:hover:bg-background"
        ...
        <div className="px-4 mt-5">
          ...
          <div className="space-y-2">
            {pendingOrders!.map((order) => (
              <div
                key={order.id}
                className="flex items-center gap-3 p-3.5 rounded-xl bg-card text-card-foreground border border-border shadow-sm"
              >
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <ArrowRight className="h-3.5 w-3.5 text-muted-foreground dark:text-muted-foreground" />
                </div>
              </div>
            ))}
          </div>
...
function MiniStat({ label, value, color, icon: Icon }: MiniStatProps) {
  return (
    <div className="rounded-xl bg-card text-card-foreground border border-border shadow-sm p-3 flex flex-col gap-2">
      <div
        className={cn(
