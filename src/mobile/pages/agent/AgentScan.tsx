...
export function AgentScan({ onGoRecord, onGoVisit, onOpenStore }: Props) {
  ...
  const [unknownUpi, setUnknownUpi] = useState<{
    upiId: string;
    payeeName: string;
  } | null>(null);
  ...
  const [nearbyStores, setNearbyStores] = useState<
    Array<StoreOption & { _distKm: number }>
  >([]);
  ...
  const handleQrScan = useCallback(
    async (data: string) => {
      ...
      const { data: qrRecord } = await supabase
        .from("store_qr_codes")
        .select(
          "store_id, stores(id, name, display_id, photo_url, outstanding, address, phone, lat, lng, route_id, is_active, store_type_id, customer_id, customers(name), store_types(name), routes(name))",
        )
        .eq("upi_id", upi.pa)
        .maybeSingle();
  ...
  const handleFindNearby = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from("stores")
        .select(
          "id, name, display_id, photo_url, outstanding, store_type_id, customer_id, lat, lng, address, phone, route_id, is_active, customers(name), store_types(name), routes(name)",
        )
        .eq("is_active", true)
        .not("lat", "is", null)
        .not("lng", "is", null);
  ...
  const renderQuickActions = () => (
    <div className="grid grid-cols-3 gap-2">
      ...
      <button
        ...
        className="flex flex-col items-center gap-2 p-4 rounded-2xl bg-card text-card-foreground border border-border hover:bg-muted dark:hover:bg-slate-750 active:scale-95 transition-all shadow-sm"
      >
      ...
      <button
        onClick={() =>
          selectedStore ? handleVisitStore(selectedStore) : onGoVisit()
        }
        disabled={visitLoading}
        className="flex flex-col items-center gap-2 p-4 rounded-2xl bg-card text-card-foreground border border-emerald-200 dark:border-emerald-800/40 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 active:scale-95 transition-all shadow-sm disabled:opacity-60"
      >
        {visitLoading ? (
        ...
        <span className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 leading-tight text-center">
          {selectedStore ? "Mark Visited" : "Routes & Visits"}
        </span>
      </button>
    </div>
  );
    ...
    <div className="flex flex-col min-h-full">
          ...
          {!hasResult && !scannerError && (
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <div
                ...
                style={{
                  maskImage:
                    "radial-gradient(ellipse 260px 260px at center, transparent 40%, black 70%)",
                  WebkitMaskImage:
                    "radial-gradient(ellipse 260px 260px at center, transparent 40%, black 70%)",
                }}
              />
                ...
                {nearbyStores.map((store) => (
                  ...
                  >
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {store.display_id} ·{" "}
                      {store._distKm < 1
                        ? `${Math.round(store._distKm * 1000)}m`
                        : `${store._distKm.toFixed(1)}km`}
                    </p>
                  </button>
                ))}
              </div>
      ...
      {selectedStore && (
        <div className="px-4 pt-3">
          <div className="rounded-2xl bg-card text-card-foreground border border-emerald-100 dark:border-emerald-800/40 shadow-sm overflow-hidden">
            ...
            <div className="p-4 flex items-start gap-3">
              <button
                ...
                onClick={() => onOpenStore(selectedStore)}
              >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  {selectedStore.store_types?.name && (
                    <Badge
                      variant="secondary"
                      className="text-[10px] font-semibold bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 border-0"
                    >
                      {selectedStore.store_types.name}
                    </Badge>
                  )}
                  {selectedStore.routes?.name && (
                    <Badge
                      variant="secondary"
                      className="text-[10px] font-semibold bg-muted hover:bg-accent text-muted-foreground border-0"
                    >
                      {selectedStore.routes.name}
                    </Badge>
                  )}
              ...
              <div className="text-right shrink-0">
                ...
                <p
                  className={cn(
                    "font-bold text-base",
                    selectedStore.outstanding > 0
                      ? "text-red-500"
                      : "text-emerald-500",
                  )}
                >
                  ₹{Number(selectedStore.outstanding).toLocaleString("en-IN")}
                </p>
              </div>
            </div>
      ...
      <div className="p-4 pt-3">
        <div className="rounded-2xl bg-card text-card-foreground border border-border shadow-sm p-4">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-foreground mt-1">
                {selectedStore
                  ...
                  : "Use the same sale and payment flow as the web app"}
              </p>
            </div>
            {selectedStore && (
              <Badge
                variant="outline"
                className="text-[10px] font-semibold border-emerald-200 dark:border-emerald-700 text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20"
              >
                Store selected
              </Badge>
            )}
          ...
          {!selectedStore && (
            <p className="text-[11px] text-muted-foreground mt-3 leading-relaxed">
              If you start before scanning, the next screen lets you choose the
              store manually.
            </p>
          )}
        </div>
      ...
      {unknownUpi && !selectedStore && (
        <div className="p-4 pt-0 space-y-3">
          <div className="rounded-2xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/40 p-4">
            ...
            <p className="font-mono text-sm text-foreground break-all bg-card text-card-foreground px-3 py-2 rounded-xl">
              {unknownUpi.upiId}
            </p>
            ...
            <p className="text-xs text-amber-600/80 dark:text-amber-500/80 mt-3 leading-relaxed">
              This UPI ID is not linked to any store. Ask your admin to register
              it.
            </p>
          </div>
          <Button
