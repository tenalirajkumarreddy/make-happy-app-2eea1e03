  ...
  const { data: sales, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("sales")
        .select(
          "id, display_id, total_amount, cash_amount, upi_amount, credit_amount, created_at, stores(name), customers(name)",
        )
  ...
  return (
    <div className="pb-8 bg-muted dark:bg-[#0f1115] min-h-full">
      ...
      <div className="px-5 space-y-4">
        ...
        <div className="bg-background dark:bg-[#1a1d24] rounded-2xl p-2 shadow-sm flex items-center pr-3 border-transparent focus-within:border-violet-500 dark:focus-within:border-violet-500 transition-colors border">
        ...
        {isLoading ? (
          ...
          <div className="bg-background dark:bg-[#1a1d24] rounded-2xl py-12 text-center shadow-sm">
            <div className="h-14 w-14 bg-muted rounded-full flex items-center justify-center mx-auto mb-3">
              <ShoppingCart className="h-6 w-6 text-muted-foreground dark:text-muted-foreground" />
            </div>
          ...
          <div className="space-y-3">
            {filtered.map((sale: any) => (
              <div
                key={sale.id}
                className="bg-background dark:bg-[#1a1d24] rounded-2xl shadow-sm p-4 active:scale-[0.98] transition-all cursor-pointer border border-transparent hover:border-border dark:hover:border-border"
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="bg-secondary text-secondary-foreground px-2 py-1 rounded-md shrink-0">
                ...
                <div className="pt-3 border-t border-border/50 flex items-end justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      {Number(sale.cash_amount || 0) > 0 && (
                        <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-1.5 py-0.5 rounded">
                          ...
                          <span className="opacity-70">
                            ₹{Number(sale.cash_amount).toLocaleString("en-IN")}
                          </span>
                        </span>
                      )}
                      {Number(sale.upi_amount || 0) > 0 && (
                        <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-500/10 px-1.5 py-0.5 rounded">
                          UPI{" "}
                          <span className="opacity-70">
                            ₹{Number(sale.upi_amount).toLocaleString("en-IN")}
                          </span>
                        </span>
                      )}
                    </div>
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-[10px] font-bold text-muted-foreground mb-0.5 uppercase tracking-widest">
                      Total
                    </p>
