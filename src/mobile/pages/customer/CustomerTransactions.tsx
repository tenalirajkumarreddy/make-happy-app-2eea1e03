    ...
    <div className="px-4 pt-4 pb-6 space-y-3">
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
            <div className="grid grid-cols-5 gap-2 px-3 py-2 text-[10px] font-bold text-slate-500 uppercase tracking-widest bg-slate-100 dark:bg-slate-800 rounded-xl">
                 <div className="col-span-1">Date</div>
                 <div className="col-span-2">Desc</div>
                 <div className="text-right">Debit</div>
                 <div className="text-right">Credit</div>
                 {/* <div className="text-right">Balance</div> - hidden on mobile due to space, or maybe stacked */}
            </div>
            
          {ledger.map((entry) => (
            <div key={`${entry.type}-${entry.id}`} className="rounded-xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 p-3 shadow-sm">
                <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-semibold text-slate-800 dark:text-slate-200">
                        {new Date(entry.date).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "2-digit" })}
                    </span>
                    <span className="text-[10px] text-slate-400">{entry.displayId}</span>
                </div>
                
                <div className="flex items-start justify-between gap-3 text-xs">
                     <div className="flex-1">
                        <p className="font-medium text-slate-700 dark:text-slate-300">{entry.storeName}</p>
                        <p className="text-[10px] text-slate-500 capitalize">{entry.type}</p>
                     </div>
                     
                     <div className="text-right space-y-0.5">
                        {entry.type === "delivery" && (
                             <p className="text-rose-600 font-medium">Dr. ₹{entry.saleAmount.toLocaleString("en-IN")}</p>
                        )}
                        {entry.type === "payment" && (
                             <p className="text-emerald-600 font-medium">Cr. ₹{entry.paidAmount.toLocaleString("en-IN")}</p>
                        )}
                        <p className="text-[10px] text-slate-500">Bal: ₹{entry.newOutstanding.toLocaleString("en-IN")}</p>
                     </div>
                </div>
            </div>
          ))}
