...
const PayrollDetailPage = () => {
  ...
  const processPayrollMutation = useMutation({
    mutationFn: async () => {
        if (!payrollId) return;
        const { error } = await (supabase as any).rpc('process_payroll', { p_payroll_id: payrollId });
        if (error) throw error;
    },
    onSuccess: () => {
  ...
  const totalAmount = (payrollItems ?? []).reduce((sum: any, item: any) => sum + item.amount, 0) || 0;
    ...
    <div className="container mx-auto py-10">
      <Card className="mb-6">
        <CardHeader>
          <CardTitle className="flex justify-between items-start">
            <span>Payroll Run: {p.display_id}</span>
          ...
          <CardDescription>{p.notes || "No notes for this payroll run."}</CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-3 gap-4">
            <div className="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p className="font-semibold">{new Date(p.start_date).toLocaleDateString()} - {new Date(p.end_date).toLocaleDateString()}</p>
                </div>
            </div>
            <div className="flex items-center space-x-3">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div>
            ...
            {p.status === 'draft' && payrollItems && payrollItems.length > 0 && (
