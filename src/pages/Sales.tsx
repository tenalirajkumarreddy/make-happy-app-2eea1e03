      ...
      <div className="flex flex-wrap md:flex-nowrap items-center gap-2 p-3 rounded-lg border bg-muted/30">
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline" className="h-8 text-xs gap-1.5 justify-start font-normal w-full sm:w-[90px] md:w-[100px]">
          ...
          {selectedSale && (
            <div className="space-y-4">
              ...
              <div className="rounded-lg border bg-muted/30 p-3 space-y-1 text-sm">
                ...
                <div className="flex justify-between font-medium"><span>Outstanding</span><span className={Number(selectedSale.outstanding_amount || 0) > 0 ? "text-destructive" : Number(selectedSale.outstanding_amount || 0) < 0 ? "text-green-600" : "text-muted-foreground"}> {Number(selectedSale.outstanding_amount || 0) < 0 ? '-' : ''}₹{Math.abs(Number(selectedSale.outstanding_amount || 0)).toLocaleString()}</span></div>
              </div>
