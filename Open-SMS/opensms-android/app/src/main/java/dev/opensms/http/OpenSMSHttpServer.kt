...
class OpenSMSHttpServer(
        ...
        return try {
            ...
            when {
                ...
                uri.startsWith("/status/") && method == Method.GET -> {
                    val msgId = uri.removePrefix("/status/")
                    handleStatus(msgId)
                }
    ...
    private fun handleHealth(): Response {
        ...
        val json = JSONObject().apply {
            ...
            put("queue_depth", messageChannel.isEmpty.let { if (it) 0 else 1 })
    ...
    private fun handleSend(session: IHTTPSession): Response {
        ...
        if (!to.matches(Regex("^\\+[1-9]\\d{6,14}$"))) {
            return errorResponse(400, "invalid_number", "Number must be in E.164 format")
        }
        ...
        val renderedBody: String = when {
            templateName != null -> {
                ...
                try {
                    ...
                    return errorResponse(400, "missing_variable", "Missing: ${e.variables.joinToString()}")
                }
            }
        ...
        if (isPaused) {
            return errorResponse(503, "gateway_paused", "Gateway is paused")
        }
        ...
        if (sendResult.isFailure) {
            return errorResponse(503, "queue_full", "Message queue is full — retry later")
        }
    ...
    private fun handleTemplates(): Response {
        ...
        val json = JSONObject().put("templates", arr)
        return newFixedLengthResponse(Response.Status.OK, MIME_JSON, json.toString())
    }

    private fun handlePause(session: IHTTPSession): Response {
        val body = parseBody(session)
        isPaused = body.optBoolean("paused", isPaused)
        val json = JSONObject().put("paused", isPaused)
        return newFixedLengthResponse(Response.Status.OK, MIME_JSON, json.toString())
    }
