...
import {
  StorePickerSheet,
  StoreOption,
} from "@/mobile/components/StorePickerSheet";
...
function RecordSale({
  ...
  const { data: availableProducts, isLoading: loadingProducts } = useQuery({
    ...
    queryFn: async () => {
      ...
      typePricing?.forEach((p: any) => {
        typePriceMap[p.product_id] = Number(p.price);
      });
      ...
      storePricing?.forEach((p: any) => {
        storePriceMap[p.product_id] = Number(p.price);
      });
  ...
  const addItem = (productId: string) => {
    ...
    if (exists) {
      ...
      if (p)
        setItems([
          ...items,
          { product_id: productId, quantity: 1, unit_price: p.effectivePrice },
        ]);
    }
  };
  ...
  const { data: storeTypes } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("store_types")
        .select("id, credit_limit_kyc, credit_limit_no_kyc");
      return data || [];
    },
  });
  ...
  const { data: customers } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("customers")
        .select("id, kyc_status, credit_limit_override");
      return data || [];
    },
  });

  const creditLimitInfo =
    store && storeTypes && customers
      ? resolveCreditLimit(store, storeTypes as any[], customers as any[])
      : null;

  const creditExceeded = !!(
    creditLimitInfo &&
    creditLimitInfo.limit > 0 &&
    newOutstanding > creditLimitInfo.limit
  );
  const creditWarning = !!(
    creditLimitInfo &&
    creditLimitInfo.limit > 0 &&
    newOutstanding > creditLimitInfo.limit * 0.8 &&
    !creditExceeded
  );
  ...
  const validateAndConfirm = () => {
    ...
    if (role === "pos" && outstandingFromSale !== 0) {
      toast.error(
        "POS sales require full payment. Cash + UPI must equal total amount.",
      );
      return;
    }
    if (creditExceeded) {
  ...
  const handleSubmit = async () => {
    ...
    if (role === "agent" && store) {
      ...
      if (locSetting?.value === "true") {
        ...
        if (result.skippedNoGps)
          toast.warning("Store has no GPS — location check skipped");
      }
    }
    ...
    logActivity(
      ...
      String(displayId),
      undefined,
      { total: totalAmount, store: store.id },
    );
  ...
  return (
    <div className="space-y-4 pb-6">
        ...
        <button
          className={cn(
            ...
            store
              ...
              : "border-dashed border-border hover:border-blue-200 dark:hover:border-blue-700 hover:bg-muted dark:hover:bg-card/50",
          )}
          onClick={() => setStorePickerOpen(true)}
          aria-label={
            store ? `Change store, currently ${store.name}` : "Select a store"
          }
        >
          <div
            className={cn(
          ...
          >
            <StoreIcon
              className={cn(
                "h-5 w-5",
                store ? "text-blue-500" : "text-muted-foreground",
              )}
            />
          </div>
          {store ? (
          ...
          <ChevronRight
            className={cn(
              "h-4 w-4 shrink-0",
              store ? "text-blue-400" : "text-muted-foreground",
            )}
          />
        </button>
      </div>
      ...
      {store && (
        <div className="px-4">
          <div className="rounded-2xl bg-card text-card-foreground border border-border p-3.5 flex justify-between items-center">
            <div>
              ...
              <p
                className={cn(
                  ...
                  oldOutstanding > 0 ? "text-red-500" : "text-emerald-500",
                )}
              >
      ...
      {/* Entry options */}
      <div className="px-4">
        <div className="rounded-2xl bg-card text-card-foreground border border-border p-3 space-y-2.5">
          {canRecordBehalf && (
            <div>
              ...
              <select
                ...
                className="mt-1 w-full h-10 rounded-xl border border-border bg-background text-foreground px-3 text-sm"
              >
                      ...
                      {inCart ? (
                        <div className="flex items-center gap-2 shrink-0">
                          <span className="text-xs text-muted-foreground font-medium">
                            ₹
                            {(
                              inCart.quantity * inCart.unit_price
                            ).toLocaleString("en-IN")}
                          </span>
                          <button
                            className="h-8 w-8 rounded-xl border-2 border-border flex items-center justify-center hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
      ...
      {items.length > 0 && (
        <div className="px-4 space-y-4">
          {/* Cart summary */}
          <div className="rounded-2xl bg-card text-card-foreground border border-border p-4">
                  ...
                  >
                    ...
                    {canOverridePrice ? (
                      <Input
                        ...
                        onChange={(e) =>
                          updateUnitPrice(item.product_id, e.target.value)
                        }
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-2xl bg-card text-card-foreground border border-border p-3">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    placeholder="0"
                    className="pl-7 h-11 rounded-xl text-base font-semibold border-border"
                  />
                </div>
              </div>
              <div className="rounded-2xl bg-card text-card-foreground border border-border p-3">
          ...
          >
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-xs">
                ...
                >
                  {outstandingFromSale >= 0 ? "+" : ""}₹
                  {outstandingFromSale.toLocaleString("en-IN")}
                </span>
              </div>
              <div className="flex justify-between text-sm font-bold border-t border-border pt-2 mt-1">
                ...
                <span
                  className={cn(
                    "text-base",
                    newOutstanding > 0 ? "text-red-500" : "text-emerald-500",
                  )}
                >
          ...
          {creditExceeded && (
            <div className="flex items-center gap-2.5 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700/40 rounded-2xl px-4 py-3">
              ...
              <span className="text-xs font-medium text-red-700 dark:text-red-400">
                Credit limit exceeded ({creditLimitInfo?.source}). Reduce items
                or collect more payment.
              </span>
            </div>
          )}
...
function RecordPayment({
  ...
  const handleSubmit = async () => {
    ...
    logActivity(
      ...
      String(displayId),
      undefined,
      { total: totalPayment, store: store.id },
    );
    getAdminUserIds().then((ids) => {
      const others = ids.filter((id) => id !== user!.id);
      if (others.length > 0) {
  ...
  return (
    <div className="space-y-4 pb-6">
        ...
        <button
          className={cn(
            ...
            store
              ...
              : "border-dashed border-border hover:border-emerald-200 dark:hover:border-emerald-700 hover:bg-muted dark:hover:bg-card/50",
          )}
          onClick={() => setStorePickerOpen(true)}
        >
          <div
          ...
          >
            <StoreIcon
              className={cn(
                "h-5 w-5",
                store ? "text-emerald-500" : "text-muted-foreground",
              )}
            />
          </div>
          {store ? (
          ...
          <ChevronRight
            className={cn(
              "h-4 w-4 shrink-0",
              store ? "text-emerald-400" : "text-muted-foreground",
            )}
          />
        </button>
      </div>
      ...
      <div className="px-4">
        ...
        <div className="mt-2">
          <div className="rounded-xl bg-card text-card-foreground border border-border px-3 py-2.5">
            <Input
              value={notes}
        ...
        <div className="mt-2 rounded-2xl bg-card text-card-foreground border border-border p-3 space-y-2.5">
          {canRecordBehalf && (
            <div>
              <Label className="text-xs text-muted-foreground font-semibold">
                Record For
  ...
  return (
    <div className="pb-4">
      ...
      <div className="bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-700 dark:from-slate-900 dark:via-blue-950 dark:to-indigo-950 px-4 pt-4 pb-6">
        <p className="text-blue-200 text-xs font-medium uppercase tracking-widest mb-3">
          Action
        </p>
        <div className="bg-background/15 backdrop-blur-sm rounded-2xl p-1 flex gap-1">
          {allowSale && (
            <button
      ...
      <div className="mt-4">
        {allowSale && activeTab === "sale" && (
          <RecordSale preselectStore={preselectStore} />
        )}
        {allowPayment && activeTab === "payment" && (
