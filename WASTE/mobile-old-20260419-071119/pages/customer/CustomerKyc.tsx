...
const DOC_FIELDS: {
...
}[] = [
  {
    ...
    field: "kyc_selfie_url",
    icon: <Camera className="h-5 w-5" />,
  },
  {
    label: "Aadhaar Front",
    hint: "Front side showing name and number",
...
export function CustomerKyc() {
  ...
  const { data: customer, isLoading } = useQuery<CustomerKycData | null>({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("customers")
        .select(
          "id, name, kyc_status, kyc_selfie_url, kyc_aadhar_front_url, kyc_aadhar_back_url, kyc_rejection_reason, kyc_submitted_at",
        )
        .eq("user_id", user!.id)
        .maybeSingle();
  ...
  const uploadDoc = async (field: KycField, file: File) => {
    ...
    if (!allowed.includes(file.type)) {
      toast.error("Only JPG, PNG, HEIC or WebP images allowed.");
      return;
    }
    ...
    if (
      current.kyc_selfie_url &&
      current.kyc_aadhar_front_url &&
      current.kyc_aadhar_back_url
    ) {
      updatePayload.kyc_status = "pending";
    ...
    if (updatePayload.kyc_status === "pending") {
  ...
  const handleFileChange =
    (field: KycField) => async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-3 px-6 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          No customer account linked to your profile.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-muted  pb-24">
      <div className="bg-background text-foreground border-b border-border px-4 pt-10 pb-5 safe-top">
      ...
      <div className="px-4 py-5 space-y-4">
          ...
          <div className="flex items-center gap-3 rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 px-4 py-4">
            ...
            <div>
              ...
              <p className="text-sm text-emerald-600 dark:text-emerald-400">
                Your identity has been verified. You enjoy higher credit limits.
              </p>
            </div>
          </div>
        )}
        ...
        {!isVerified && !isPending && (
          <div className="rounded-2xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 px-4 py-3">
            ...
            <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
              Verified customers enjoy higher credit limits and faster order
              approvals.
            </p>
          </div>
        )}
        ...
        <div className="space-y-3">
          {DOC_FIELDS.map(({ label, hint, field, icon }) => {
            ...
            return (
              <div
                key={field}
                className="rounded-2xl bg-background text-foreground border border-border shadow-sm overflow-hidden"
              >
                <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-50 dark:border-border">
                  <div
                    className={cn(
                      "h-9 w-9 rounded-xl flex items-center justify-center",
                      hasDoc
                        ? "bg-emerald-100 text-emerald-600"
                        : "bg-slate-100 text-muted-foreground dark:bg-card dark:text-muted-foreground",
                    )}
                  >
                    {icon}
                  </div>
        ...
        {canUpload && (
          <p className="text-xs text-center text-muted-foreground leading-relaxed px-4">
            Upload all three documents to submit your KYC for review. Files must
            be under 10MB each.
          </p>
        )}
      </div>
